defmodule BsonTk do
  ...
  * `tk_end` - is the end position of the element
  """
  defp tokenize_element(<<0x10>>, _, from), do: {%Int32{part: {from, 4}}, from+4}
  defp tokenize_element(<<0x12>>, _, from), do: {%Int64{part: {from, 8}}, from+8}
  ...
  defp tokenize_element(<<0xff>>, _, from), do: {MIN_KEY, from+0}
  defp tokenize_element(<<0x7f>>, _, from), do: {MAX_KEY, from+0}

  # string
  defp tokenize_element(<<0x02>>, bson, from) do
    len = Bson.int32(bson, from)-1
    {%String{part: {from+4, len}}, from+len+4+1}
  end

  # atom
  defp tokenize_element(<<0x0e>>, bson, from) do
    len = Bson.int32(bson, from)-1
    {%Atom{part: {from+4, len}}, from+len+4+1}
  end

  # document
  defp tokenize_element(<<0x03>>, bson, from) do
    len = Bson.int32(bson, from)-5
    {%Doc{part: {from+4, len}}, from+len+4+1}
  end

  # array
  defp tokenize_element(<<0x04>>, bson, from) do
    len = Bson.int32(bson, from)-5
    {%Array{part: {from+4, len}}, from+len+4+1}
  end

  # regex
  defp tokenize_element(<<0x0b>>, bson, from) do
    to = byte_size(bson) -1
  ...
  # javascript
  defp tokenize_element(<<0x0d>>, bson, from) do
    len = Bson.int32(bson, from)-1
    {%BsonTk.JS{code: {from+4, len}}, from+len+4+1}
  end
  # javascript with scope
  defp tokenize_element(<<0x0f>>, bson, code_ws_from) do
  ...
  # binary
  defp tokenize_element(<<0x05>>, bson, from) do
